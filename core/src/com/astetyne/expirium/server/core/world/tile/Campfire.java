...
public class Campfire implements PlayerInteractListener, TickListener, Saveable {
    ...
    public void onTick(float delta) {
        /*remainingTime -= delta;
        if(remainingTime <= 0) {
            server.getWorld().changeMaterial(tile, Material.AIR, false, Source.NATURAL);
        ...
        }else if(tile.getMaterial() == Material.CAMPFIRE_BIG && remainingTime < 60) {
            server.getWorld().changeMaterial(tile, Material.CAMPFIRE_SMALL, false, null, Source.NATURAL);
        }
        inventory.onTick();*/
    }
