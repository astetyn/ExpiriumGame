...
public class Frog extends LivingEntity {
    ...
    private final static byte MAX_HEALTH = 20;

    private final Vector2 originLoc;
    private long nextAITick;

    public Frog(ExpiServer server, Vector2 loc) {
        super(server, EntityType.FROG, loc, MAX_HEALTH);
        originLoc = new Vector2(loc);
        nextAITick = server.getWorld().getTick();
    }

    public Frog(ExpiServer server, DataInputStream in) throws IOException {
        super(server, EntityType.FROG, MAX_HEALTH, in);
    ...
    public void onTick() {
        ...
        if(server.getWorld().getTick() >= nextAITick) {
            Vector2 impulse;
            if(Math.random() > 0.5) {
                impulse = new Vector2((float) (Math.random()*150), (float) (Math.random()*100));
            }else {
                impulse = new Vector2((float) -(Math.random()*150), (float) (Math.random()*100));
            }
